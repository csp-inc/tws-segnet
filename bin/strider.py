...
class Image:
    def __init__(self, filename: str, model_done: bool = False):
        ...
        if '.tif' in self.filename:
            ...
            if self.model_done:
                ...
                self.array = self.img.ReadAsArray()
    ...
    def parse_dims(self) -> list:
        ...
        if self.numdims < 3:
            return [None, self.array.shape[0], self.array.shape[1]]
        else:
    ...
    def make_windows(self) -> np.ndarray:
        ...
        windows = np.ndarray((self.yinds.size, self.xinds.size), dtype=Window)
        ...
        if self.model_done:
            for yix, y in enumerate(self.yinds):
                for xix, x in enumerate(self.xinds):
                    windows[yix, xix] = Window(self, xix, x, yix, y)
        else:
            if self.banddim == 0 or not self.banddim:
                for yix, y in enumerate(self.yinds):
                    for xix, x in enumerate(self.xinds):
                        padding = {}
    ...
    def reassemble_windows(self) -> np.ndarray:
        ...
        bigcat = []
        colcount = self.windows.shape[0] - self.chop_last_col
        rowcount = self.windows.shape[1] - self.chop_last_row
